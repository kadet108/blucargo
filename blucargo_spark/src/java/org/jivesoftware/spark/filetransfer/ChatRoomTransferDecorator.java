...
public class ChatRoomTransferDecorator implements KeyListener, FileDropListener, ChatRoomClosingListener, ActionListener {
    ...
    public ChatRoomTransferDecorator(final ChatRoom chatRoom) {
        ...
        sendFileButton = new CargoChatRoomButton("", SparkRes.getImageIcon(SparkRes.SPARK_IKONKA_WYSLIJ));
        ...
        //za pomoca tych dwoch linii zmieniam odstepy ale to chyba nie do konca dobrze
        sendFileButton.setMinimumSize(new Dimension(70, 100));
        sendFileButton.setPreferredSize(new Dimension(70, 100));
        chatRoom.getToolBar().addChatRoomButton(sendFileButton);

        sendScreenShotButton = new CargoChatRoomButton("", SparkRes.getImageIcon(SparkRes.SPARK_CAPTURE_SCREEN));
        sendScreenShotButton.setToolTipText(Res.getString("message.send.picture"));
        sendScreenShotButton.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
