...
public class OfferConfirmationWindow extends JPanel {
	...
	JPanel window 			= this;
	JPanel parentWindow		= null;
	
	CargoOffer cargoOffer;

	public OfferConfirmationWindow(CargoOffer cargoOffer, JPanel window) {
	...
	private void createMain() {
		...
		String shortDescriptionString = String.format("%s %s %s", cargoOffer.getWeight(),cargoOffer.getBody(),cargoOffer.getCargoLength());
		JLabel shortDescription = new JLabel(shortDescriptionString );

		String hds = cargoOffer.getHds();
		...
		String fromCountry = cargoOffer.getCountryFrom();
		String fromCity = cargoOffer.getCityFrom();
		String fromStreet = cargoOffer.getAddressFrom();
		String fromZIP = cargoOffer.getPostOfficeFrom();
		...
		String toCountry = cargoOffer.getCountryTo();
		String toCity = cargoOffer.getCityTo();
		String toStreet = cargoOffer.getAddressTo();
		String toZIP = cargoOffer.getPostOfficeTo();
		...
		if (cargoOffer.getType() == OfferType.CARGO) {
			coJestWolne.setText("<html><b>Wolny \u0142adunek</b></html>");
		...
		column2.add(shortDescription, 			new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.NORTH, GridBagConstraints.BOTH, new Insets(0, 10, 0, 10), 0, 0));
	...
	private void createFooter(ImageIcon pasekStopkaTlo) {
		...
		JButton akceptuj = new RolloverButton(SparkRes.getImageIcon(SparkRes.CARGO_AKCEPTUJ));
		akceptuj.addActionListener(new ActionListener(){

			@Override
			public void actionPerformed(ActionEvent e) {
				CargoDataManager.getInstance().addCargoOffer(cargoOffer);
				CargoDataManager.getInstance().refreshAcceptedOffers();
				
				BlucargoSortableTableModel model = (BlucargoSortableTableModel) CargoDataManager.getInstance().getCargoOffersTable().getModel();
				model.refreshSlowly();
				
				BlucargoPlugin.killTab(window);
				window = null;
			}
			
		});
		
		JButton anuluj = new RolloverButton(SparkRes.getImageIcon(SparkRes.CARGO_ANULUJ));
		anuluj.addActionListener(new ActionListener() {
		 ...
		 windowFooter.add(akceptuj, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.BOTH,
		 new Insets(0, 0, 0, 0), 0, 0));
		 windowFooter.add(anuluj, new GridBagConstraints(1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.BOTH,
		 new Insets(0, 0, 0, 0), 0, 0));
	...
	private void createHeader(ImageIcon pasekMenuTlo, ImageIcon logoIcon, ImageIcon szczegolyIcon) {
		...
		// JLabel takaNazwa = new
		// JLabel("<html><font color=white size=6>Szczeg\ufffdly ogloszenia</font></html>");

		// windowHeader.add(new JLabel(logoIcon), new GridBagConstraints(0, 0,
		// 1, 1, 0.0, 0.0,
...
//				new OfferWindowConfirmation(co).setVisible(true);
			}
		});
	}
